...
struct p2m_pcache_miss_msg {
	...
	__u32			flags;
	__u64			missing_vaddr;
	__u8			teleport_cmd; // embed a TELEPORT operation
};
...
/*
P2M_INVALID: invalidate a page for a Teleport instance on memory node
*/

struct p2m_pcache_invalid_payload {
	__u32			pid;
	__u32			tgid;
	__u64			invalid_vaddr;
};
...
int handle_p2m_msync(struct p2m_msync_struct *, struct common_header *, struct thpool_buffer *);
...
struct p2m_pushdown_pageinfo {
    __u64	addr;
    char	is_dirty;
};

struct p2m_pushdown_pageinfo_addr_compression {
    __u64	start_addr;
	__u32	count;
};

struct p2m_pushdown_pageinfo_dirty_compression {
	char	is_dirty;
	__u32	count;
};

struct p2m_pushdown_pageinfo_compression {
    __u64	start_addr_dirty_embedded;
	__u32	count;
};

struct p2m_pushdown_payload_binary {
	__u32	pid;
	__u32	payload_size;
	char	filename[MAX_FILENAME_LENGTH];
	__u32	argc;
...
struct p2m_pushdown_payload_fix {
	__u32	pid;
	char	filename[MAX_FILENAME_LENGTH];
	__u32	argc;
	__u32	envc;
...
void handle_p2m_pushdow(struct p2m_pushdown_payload *payload, struct common_header *hdr, struct thpool_buffer *tb);

struct p2m_syncmem_m2p_msg {
	struct common_header	header;
	__u32			pid;
	__u32			tgid;
	__u32			flags;
...
void handle_p2m_syncmem(struct p2m_syncmem_m2p_msg* msg, struct thpool_buffer *b);
