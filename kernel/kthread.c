...
/**
 * Periodically print a heartheat message to prevent remote console from being timed out. 
 * 
*/
int kheartbeat(void *unused) {
	struct task_struct *tsk = current;
	u64 lifetime = 0;

	/* Setup a clean context for our children to inherit. */
	set_task_comm(tsk, "kheartbeat");

	for (;;) {
		// sleep 5 min
		msleep(300000);

		printk("Beep: kernel has lived for %lu minutes\n", (lifetime + 1) * 5);
