...
DEFINE_SPINLOCK(teleport_pagefault_lock);
struct accessinfo* compute_pool_access_list;

#ifdef TELEPORT_PRINTING
unsigned long num_compute_invalidation_success = 0;
unsigned long num_compute_invalidation_retry = 0;
unsigned long num_compute_invalidation_send = 0;
#endif
		      ...
		      struct pcache_meta *pcm, void *unused)
{
	...
	if (PcachePiggyback(pcm)) {
		...
		// should check whether the current tgid matches those pushdown tgids
		if (teleport_reqs) {
			if (flags & FAULT_FLAG_WRITE) {
				pb_msg->miss.teleport_cmd = T_CMD_PAGE_INVALIDATE;
			} else {
				pb_msg->miss.teleport_cmd = T_CMD_PAGE_WRPROTECT;
			}
		} else {
			pb_msg->miss.teleport_cmd = 0;
		}
...
fallback:
		...
		if (teleport_reqs) {
			if (flags & FAULT_FLAG_WRITE) {
				msg.teleport_cmd = T_CMD_PAGE_INVALIDATE;
			} else {
			    msg.teleport_cmd = T_CMD_PAGE_WRPROTECT;
			}
		} else {
			msg.teleport_cmd = 0;
		}
	...
	if (unlikely(len < (int)PCACHE_LINE_SIZE)) {
		if (likely(len == sizeof(int))) {
			 ...
			 */
			ret = len;
            printk("Page fault request timed out (%#lx)\n", address);
			WARN_ON_ONCE(1);
			goto out;
		} else {
...
/*
	For page invalidation in memory pool in TELEPORT.
*/
static inline int pcache_do_invalidate_page(unsigned long address) {
	__u32 len_msg;
	int retlen;
    void *msg;
    struct common_header *hdr;
    struct p2m_pcache_invalid_payload *payload;
	int res;
	__u32 node_id;
	int pid, tgid;
	...
	pid = current->pid;
	tgid = current->tgid;
	node_id = get_memory_node(current, address);
	len_msg = sizeof(struct common_header) + sizeof(struct p2m_pcache_invalid_payload);
	...
	hdr = msg;
    hdr->opcode = P2M_PCACHE_INVALID;
    hdr->src_nid = LEGO_LOCAL_NID;

    payload = msg + sizeof(*hdr);
	payload->pid = pid;
    payload->tgid = tgid;
    payload->invalid_vaddr = (__u64)address;

	res = 0;

	while (true) {
#ifdef TELEPORT_PRINTING
		num_compute_invalidation_send++;
#endif
		...
		if (likely(retlen == sizeof(int))) {
			if (res == 0) {
				...
				panic("TELEPORT: failed to invalidate memory node\n");
			}
		} else {
			if (unlikely(retlen != sizeof(int))) {
				printk("TELEPORT: failed to invalidate memory node for %#lx (%d)\n", address, retlen);
				printk("TELEPORT: %d online pages have been transferred\n", teleport_online_pagefaults);
#ifdef TELEPORT_PRINTING
				printk("TELEPORT: C-invalidation: %lu sends, %lu successes, %lu retries\n", num_compute_invalidation_send, num_compute_invalidation_success, num_compute_invalidation_retry);
#endif
				WARN_ON_ONCE(1);
				res = -EFAULT;
			}
...
#ifdef TELEPORT_PRINTING
	num_compute_invalidation_retry++;
#endif

	}

#ifdef TELEPORT_PRINTING
    num_compute_invalidation_success++;
#endif
				   ...
				   pte_t *pte, pmd_t *pmd, unsigned long flags)
{
	...
	// acquire TELEPORT access list lock
	if (unlikely(teleport_reqs)) {
        page_addr = address & PAGE_MASK;
        hash_value = hash_min(page_addr, 64);
        access_index =  hash_value % TELEPORT_ACCESS_TABLE_LENGTH;
        spin_lock(&compute_pool_access_list[access_index].update_lock);
        compute_pool_access_list[access_index].addr = address;
	...
	if (likely(!pte_present(entry))) {
		if (pte_none(entry)) {
			...
			if (victim_may_hit(address)) {
				if (!victim_try_fill_pcache(mm, address, pte, entry, pmd, flags)) {
                    // unlock TELEPORT
                    if (unlikely(locked)) {
                        compute_pool_access_list[access_index].addr = 0;
			 ...
			 */
			result = pcache_do_fill_page(mm, address, pte, entry, pmd, flags);
		...
		result = pcache_do_zerofill_page(mm, address, pte, entry, pmd, flags);
		...
		if (teleport_reqs) {
			// don't use page table lock to coordinate TELEPORT coherence
			// invalidate the page
			pcache_do_invalidate_page(address);
		}
	}
	...
	if (flags & FAULT_FLAG_WRITE) {
		if (likely(!pte_write(entry))) {
			result = pcache_do_wp_page(mm, address, pte, pmd, ptl, entry);
