...
/*
 * Callback for page fault response in TELEPORT in compute pool
 */
void fit_ack_reply_callback_parameterized(void *_ctx,
	void *metadata,
	int node_id, int offset, void *reply_data, int reply_size)
{
	int last_ack, ack_flag = 0;
	int reply_connection_id;
    struct imm_message_metadata *request_metadata = (struct imm_message_metadata*)metadata;
    ppc* ctx = (ppc*)_ctx;
...
#elif defined(CONFIG_COMP_PROCESSOR)
					{
						struct imm_message_metadata *tmp_pf_msg;
						
						// compute pool receives a request
                    	tmp_pf_msg = (struct imm_message_metadata *)(ctx->local_rdma_recv_rings[node_id] + offset);
						teleport_handle_messages(ctx, tmp_pf_msg,
							(void *)tmp_pf_msg + sizeof(struct imm_message_metadata),
							tmp_pf_msg->size, node_id, offset);
					}
#else
					{
