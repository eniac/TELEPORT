...
static void rest_init(void)
{
	...
	// init buffer for TELEPORT lazy sync
	init_pfault_resp();

	// TELEPORT has no instance yet
	teleport_reqs = 0;
	teleport_ongoing_list = NULL;

    // init compute pool access list
    compute_pool_access_list = kmalloc(sizeof(struct accessinfo) * TELEPORT_ACCESS_TABLE_LENGTH, GFP_KERNEL);
	if (unlikely(!compute_pool_access_list)) {
		panic("TELEPORT: failed to allocate memory for compute_pool_access_list\n");
	}
	for (i = 0; i != TELEPORT_ACCESS_TABLE_LENGTH; i++) {
		compute_pool_access_list[i].addr = 0;
		spin_lock_init(&compute_pool_access_list[i].update_lock);
	}
#endif
    ...
    // start pushdown server here.
	pid = kernel_thread(pushdown, NULL, 0);
	pushdown_task = find_task_by_pid(pid);

	// init all TELEPORT instances
	do {
		int i;
	...
	// spawn a kernel thread to periodically print a heartbeat
	kernel_thread(kheartbeat, NULL, 0);
